...
;                       present); thanks Ben
;   mgs, 11 Aug 2002: - fixed bug in Copy method: pointers in
;                       structures were not physically copied.
;                       Increased verbosity when copying with /verbose
;   mgs, 12 Aug 2002: - fixed nasty bug in copy method: object
;                       refrences inside an object caused a dangling
;                       link. Can only think of heap_gc to remedy this
;                       problem.
;   mgs, 24 Jan 2003: - fixed another bug in Copy method: Objext
;                       arrays referred to by pointers were not
...
PRO MGS_BaseObject::ErrorMessage,  $
   ...
   ;; Stop program execution if debug level is 99
   IF self.debug EQ 99 THEN stop
...
; -------------------------------------------------------------------------------------
; CopyStructure:  (private)
;   This method returns a physical copy of a structure given as
;   argument INCLUDING copies of all pointer and object tags. If the
;   structure contains other structures, this method is called
;   recursively. 
;   Note that this method is never called directly, but only from
;   CopyPointers. 

FUNCTION MGS_BaseObject::CopyStructure, s, no_objects=no_objects, verbose=verbose

   ;; Set dummy default (simple copy)
   result = s

   ;; Go through structure tags and duplicate pointers etc.
   FOR i=0L, N_Tags(s)-1 DO BEGIN
      ;; Check type of structure tag
      type = Size( s.(i), /TName )
      IF keyword_set(verbose) THEN print,'Structure copy: tag type ',type
      IF type EQ 'POINTER' THEN BEGIN
         ;; replace pointer by physical copy
         result.(i) = self->CopyPointers( s.(i) ) 
      ENDIF ELSE IF type EQ 'OBJREF' AND not Keyword_Set(no_objects) THEN BEGIN
         ;; create physical copy of object
         result.(i) = Ptr_New( s.(i)->Copy() )
      ENDIF ELSE IF type EQ 'STRUCT' THEN BEGIN 
         ;; recursive structure
         result.(i) = self->CopyStructure( s.(i), no_objects=no_objects)
      ENDIF 
   ENDFOR
...
FUNCTION MGS_BaseObject::CopyPointers, p, no_objects=no_objects, verbose=verbose
   ...
   FOR i=0L, N-1 DO BEGIN
      IF Ptr_Valid( p[i] ) THEN BEGIN
         ...
         type = Size( *p[i], /TName )
         IF keyword_set(verbose) THEN print,'Pointer copy: reference to type ',type
         IF type EQ 'POINTER' THEN BEGIN
            ;; copy pointers recursively
            IF keyword_set(verbose) THEN print,'i=',i,'; Copying pointer...' 
            result[i] = Ptr_New( self->CopyPointers( *p[i], verbose=verbose ) )
         ENDIF ELSE IF type EQ 'OBJREF' AND NOT Keyword_Set(no_objects) THEN BEGIN
            ;; create physical copy of object
            IF keyword_set(verbose) THEN print,'i=',i,'; Copying object...' 
            IF n_elements(*p[i]) EQ 1 THEN BEGIN 
               ;; single object reference
               IF Obj_Valid(*p[i]) THEN BEGIN 
                  result[i] = Ptr_New( (*p[i])->Copy(verbose=verbose) ) 
               ENDIF 
            ENDIF ELSE BEGIN 
               ;; object array
               result[i] = Ptr_New( ObjArr(n_elements(*p[i])) )
               FOR j=0L, n_elements(*p[i])-1 DO BEGIN 
                  element = (*p[i])[j]
                  IF Obj_Valid(element) THEN BEGIN 
                     (*result[i])[j] = element->Copy(verbose=verbose)
                  ENDIF 
               ENDFOR 
            ENDELSE 
         ENDIF ELSE IF type EQ 'STRUCT' THEN BEGIN 
            IF keyword_set(verbose) THEN print,'i=',i,'; Copying structure...' 
            ;; copy structure itself, then check structure tags if
            ;; they contain pointers or object references
            result[i] = Ptr_New( self->CopyStructure(*p[i], no_objects=no_objects, $
                                                    verbose=verbose) )
         ENDIF ELSE BEGIN 
            ;; copy item verbatim, whatever it is
            IF keyword_set(verbose) THEN print,'i=',i,'; Copying item of type ',type 
            result[i] = Ptr_New( *p[i] ) 
         ENDELSE 
      ENDIF
   ENDFOR
...
FUNCTION MGS_BaseObject::Copy, no_pointers=no_pointers, $
   ...
   IF self.debug LT 2 THEN Catch, theError ELSE theerror = 0
   IF theError NE 0 THEN BEGIN
      Catch, /Cancel
   ...
   IF self.debug GT 0 THEN verbose = 1
   ...
   FOR i=0L, N_Tags(ostru)-1 DO BEGIN
      ...
      IF tname EQ 'OBJREF' THEN BEGIN
         ...
         ENDIF ELSE IF not Keyword_Set(no_objects) THEN BEGIN
            ...
            IF Obj_Valid( clone.(i) ) THEN BEGIN 
               clone.(i) = self.(i)->Copy(verbose=verbose)
               heap_gc  ;; ugly, but necessary here!
            ENDIF 
         ENDIF
      ENDIF
      ;; 
