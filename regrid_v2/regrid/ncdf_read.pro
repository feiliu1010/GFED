...
;        mgs, 14 Oct 2004: - added extra cases for valid_tagname
;        mgs, 14 Feb 2005: - make sure tags are trimmed
;
;-
;-------------------------------------------------------------
;
...
function Valid_TagName, arg
      ...
      ;; special check for internal IDL commands (necessary as of IDL6)
      if strlowcase(t) eq 'case' then t = 'case_attr'
      if strlowcase(t) eq 'message' then t = 'message_attr'
...
function ncdf_attributes, ncid, varid, global=global, verbose=verbose
    ...
    for i=0L,natts-1 do begin
       ...
       if keyword_set(verbose) then begin
          print,prefix+aname+': '+strjoin(string(avalue))
       endif
    endfor
...
pro NCDF_READ, result, filename=filename, truename=truename,    $
   ...
   if n_elements(variables) gt 0 AND load_any then dovars = variables
...
PRO timetest, data, attributes=attributes, _EXTRA=extra

   file='~/data/mozart/run002/daily/mgs002_flash_07.nc'

   t0 = systime(1)
   ncdf_read, data, file=file, /all, attributes=attributes, _Extra=extra
   t1 = systime(1)
   print, t1-t0, ' seconds'
